...
- [Exclusive test execution](#exclusive-test-execution)
...
Tests that must not run in parallel with any other test can be tagged with `pabot:exclusive`.

```robot
*** Test Cases ***
My Critical Test
    [Tags]    pabot:exclusive
    # This test will never run alongside another test
    Do Something Critical
```

**How it works:**

Before the parallel execution phase begins, Pabot scans the selected tests for the `pabot:exclusive` tag. Any matching tests are removed from the normal parallel execution pool and scheduled to run **one at a time**, each in its own isolated subprocess, after all parallel tests have finished.

**Key behaviours:**

- Works at both suite level (`pabot` default) and test level (`--testlevelsplit`).
- A **mixed suite** — containing both normal and exclusive tests — is automatically split at the test level. Normal tests from that suite run in parallel; exclusive tests run alone afterwards.
- A suite where **all** tests are exclusive is removed from the parallel phase entirely; each test runs as an individual exclusive item.
- The order of exclusive tests follows the order they appear in the ordering file (or alphabetical order by default).
- Exclusive tests are executed sequentially after all parallel stages, so total execution time is the parallel phase time plus the sum of each exclusive test's duration.
...
If `tests/` contains 10 normal tests and 2 exclusive tests, Pabot will run the 10 normal tests in parallel across up to 4 workers, then run each of the 2 exclusive tests alone in sequence.
...
Pabot will insert following global variables to Robot Framework namespace. These are here to enable PabotLib functionality and for custom listeners etc. to get some information on the overall execution of pabot.
      ...
      PABOTEXECUTIONBATCHSIZE - this contains the number of items in the current execution batch.
